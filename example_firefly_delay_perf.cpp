...
FireflyDelayPerf<float, MAX, NFF> fireflies;
...
void FireflyPerfInit(float sr_)
{
    ...
    fireflies.SetFeedback(0.36f);
    fireflies.SetDensity(0.45f);
    fireflies.SetBlinkBaseRate(0.22f);
    fireflies.SetControlDiv(8); // 6 kHz control
    fireflies.SetDetuneCents(3.0f);
    fireflies.EnableDetune(true);

    // New: scale wet output post-mix
    fireflies.SetOutputGain(1.0f); // unity (try 0.8f..1.2f)
}
